...
export async function GET() {
  ...
  const result = await runtime.DB.prepare(
    `SELECT id, title, author, COALESCE(series, '') AS series, category, color,
...
export async function POST(request: Request) {
  ...
  await runtime.DB.prepare(
    `INSERT INTO books (id, title, author, series, category, color, front_image_key, back_image_key, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).bind(
    id,
